...
%global bundle_snappy="-Dsnappy.prefix=x -Dbundle.snappy=true -Dsnappy.lib=/usr/lib"
%endif
%ifarch amd64 x86_64
%global hadoop_arch Linux-amd64-64
%global bundle_snappy="-Dsnappy.prefix=x -Dbundle.snappy=true -Dsnappy.lib=/usr/lib64"
%endif
...
These projects (enumerated below) allow HDFS to be mounted (on most flavors of Unix)
as a standard file system using

%prep
%setup -q -n %{name}-%{hadoop_base_version}-src

%build
HADOOP_VERSION=%{hadoop_base_version} HADOOP_ARCH=%{hadoop_arch} \
  mvn-rpmbuild ${bundle_snappy}  \
  -Pdist -Pnative -Psrc -Dtar \
  -DskipTests -DskipTest -DskipITs \
  install
